...
/*:
 * @target MZ
 * @plugindesc (v1.0) 파이널 판타지의 시아트리듬을 기반으로 한 플러그인을 FNF 채보를 사용하게 개조한 버전.
 * @author Moghunter
 ...
 *
 * @arg chartFile
 * @desc FNF 채보 JSON 파일명 (/data/ 폴더 기준). 예: inaposter.json
 * @text Chart File (FNF JSON)
 * @default inaposter.json
 * @type string
 *
 * @arg enemyID
 ...
 * =============================================================================
 * 시아트리듬 플러그인에 FNF 채보 데이터를 이용할 수 있게 개조한 버전입니다.
 * /data 파일 안에 Kade Engine에서 사용하는 FNF 채보 JSON 파일을 넣으세요.
 * 자세한 내용은 원본 시아트리듬 플러그인 설명을 참고해주세요.
 * =============================================================================
 * 예정 사항:
 * - 동시 치기 구현 (또는 제거)
 * - 롱노트 구현
...
Moghunter.parameters = PluginManager.parameters("MOG_Theatrhythm_FNF");
...
PluginManager.registerCommand("MOG_Theatrhythm_FNF", "Theatrhythm", (data) => {
  var enemy_id = Math.min(Math.max(Number(data.enemyID) || 1, 1), $dataEnemies ? $dataEnemies.length - 1 : 999);
  var speed = Math.min(Math.max(Number(data.speed) || 1, 1), 20);
  var animation_id = Math.min(Math.max(Number(data.animationID) || 1, 1), $dataAnimations ? $dataAnimations.length - 1 : 999);
  ...
  var chart_file = String(data.chartFile || "inaposter.json");
  $gameSystem.executeTheatrhythm(enemy_id, speed, animation_id, switch_id, variable_id, random_speed, chart_file);
});
...
Scene_Map.prototype.update = function () {
  ...
  if ($gameSystem._theatrhythm_start) {
    this.execute_theatrhythm();
  }
};
...
Game_System.prototype.initialize = function () {
  ...
  this._theatrhythm_chartFile = "";
  this._theatrhythm_fnfQueue = [];
  this._theatrhythm_fnfEnemyQueue = [];
  this._theatrhythm_fnfLoaded = false;
};

//==============================
// * execute Theatrhythm
//==============================
Game_System.prototype.executeTheatrhythm = function (enemy_id, speed, animation_id, switch_id, variable_id, randomMode, chart_file) {
  ...
  $gameSystem._theatrhythm_chartFile = chart_file || "";
  $gameSystem._theatrhythm_fnfQueue = [];
  $gameSystem._theatrhythm_fnfEnemyQueue = [];
  $gameSystem._theatrhythm_fnfLoaded = false;
...
Sprite_Battler.prototype.update_theatrhythm = function () {
  if (this._battler) {
    if (this._battler._motion_damage_duration > 0) {
      this.update_motion_damage();
    }
    if (this._battler._motion_action_data[0] === 7) {
      this.update_action_move_right();
    }
    this.x += this._battler._motion_action_xy[0];
    this.y += this._battler._motion_action_xy[1];
  }
  if (this._battler.isEnemy() && $gameSystem._theatrhythm_phase === 10) {
    this.opacity -= 5;
  }
};
...
Sprite_Battler.prototype.update_motion_damage = function () {
  if (Imported.MOG_FlashDamage && this._battler._flashDamage) {
    return;
  }
  this._battler._motion_damage_xy[0] = Math.random() * 12 - 6;
  this._battler._motion_damage_duration -= 1;
  if (this._battler._motion_damage_duration <= 0) {
    this._battler._motion_damage_xy = [0, 0];
  }
...
Scene_Theatrhythm.prototype.initialize = function () {
  ...
  for (var i = 0; i < $gameParty.battleMembers().length; i++) {
    $gameParty.battleMembers()[i].recoverAll();
  }
  this._time = [0, 3000];
  this._score = [-1, 0, 0];
  this._chain_dt = [0, 0];
...
// * Load FNF Chart
//==============================
Scene_Theatrhythm.prototype.load_fnf_chart = function () {
  var chartName = $gameSystem._theatrhythm_chartFile || "";
  if (!chartName) return;
  var fileName = "data/" + (chartName.match(/\.json$/i) ? chartName : chartName + ".json");
  ...
  xhr.onload = function () {
    if (xhr.status < 400) {
      try {
        ...
        self.parse_fnf_chart(json);
      } catch (e) {
        console.warn("FNF Chart parse error:", e);
      }
    }
  };
  xhr.onerror = function () {
    console.warn("FNF Chart load failed:", fileName);
  };
  xhr.send();
};
...
//==============================
// * Parse FNF Chart
// mustHitSection=true  → rawDir 0-3: 플레이어, 4-7: 상대
// mustHitSection=false → rawDir 0-3: 상대,    4-7: 플레이어
//==============================
Scene_Theatrhythm.prototype.parse_fnf_chart = function (json) {
  var sections = json.song.notes;
  console.log("[FNF] parse_fnf_chart: 섹션 수=" + sections.length);
  var playerQueue = [];
  var enemyQueue = [];
  for (var s = 0; s < sections.length; s++) {
    ...
    for (var n = 0; n < notes.length; n++) {
      ...
      var time = note[0];
      var rawDir = note[1];
      var isPlayer = mustHit ? rawDir < 4 : rawDir >= 4;
      var dir = rawDir % 4;
      if (isPlayer) {
        playerQueue.push({ time: time, direction: dir });
      } else {
        enemyQueue.push({ time: time, direction: dir });
      }
    }
  }
  playerQueue.sort(function (a, b) {
    return a.time - b.time;
  });
  enemyQueue.sort(function (a, b) {
    return a.time - b.time;
  });
  $gameSystem._theatrhythm_fnfQueue = playerQueue;
  $gameSystem._theatrhythm_fnfEnemyQueue = enemyQueue;
  $gameSystem._theatrhythm_fnfLoaded = true;
  console.log("[FNF] 플레이어 노트=" + playerQueue.length + " 적 노트=" + enemyQueue.length);
};
...
Scene_Theatrhythm.prototype.createBattlePower = function () {
  ...
  if (Moghunter.theatrhythm_bp_visible === "true") {
    this._bp_visible = true;
  }
  this._battlePower = [];
  for (var i = 0; i < 2; i++) {
...
Scene_Theatrhythm.prototype.refresh_damage = function (i, value) {
  ...
  if (i === 1 && this._party_hp <= 0) {
    return;
  }
  if (i === 1 && !this._actorSprites[this._target_id]) {
    return;
  }
  numbers = Math.abs(value).toString().split("");
...
Scene_Theatrhythm.prototype.refresh_chain = function (sprites, value, img_data, x) {
  ...
  if (this._chain_dt[0] > this._chain_dt[1]) {
    this._chain_dt[1] = this._chain_dt[0];
  }
  numbers = Math.abs(value).toString().split("");
  for (var i = 0; i < sprites.length; i++) {
    sprites[i].visible = false;
...
Scene_Theatrhythm.prototype.update_chain = function () {
  ...
  if (this._chain_dt[0] === 0 || $gameSystem._theatrhythm_phase != 1) {
    this._chain.opacity -= 15;
  }
  if (this._chain.scale.x > 1.0) {
    this._chain.scale.x -= 0.1;
  }
  this._chain.scale.y = this._chain.scale.x;
...
Scene_Theatrhythm.prototype.update_hp_meter = function () {
  ...
  if (this._hp_meter_data[2] > this._hp_meter_data[3]) {
    this._hp_meter_data[2] = 0;
  }
...
Scene_Theatrhythm.prototype.update_time_meter = function () {
  ...
  if (this._meter_data[2] > this._meter_data[3]) {
    this._meter_data[2] = 0;
  }
...
Scene_Theatrhythm.prototype.refresh_string = function (i, type) {
  ...
  this._strings[i].scale.x = 1.0;
  this._strings[i].scale.y = 1.0;
...
Scene_Theatrhythm.prototype.createButtons = function () {
  ...
  // 적 노트 (위치는 getSpriteData에서 설정)
  this._enemy_buttons_position = [[0, 0]];
  this._sprite_enemy_buttons = new Sprite_TKeys_Enemy(this._buttons_img, this._enemy_buttons_position);
  this._spriteHudBase.addChild(this._sprite_enemy_buttons);
};
...
Scene_Theatrhythm.prototype.getSpriteData = function () {
  ...
  this._chain_number_data = [this._chain_number_img.width / 10, this._chain_number_img.height, Moghunter.theatrhythm_chain_x + Moghunter.theatrhythm_chain_number_x, Moghunter.theatrhythm_chain_y + Moghunter.theatrhythm_chain_number_y];
  for (var i = 0; i < this._chain_number.length; i++) {
  ...
  this.createButtons();
  // 적 노트 판정선 위치 설정 (적 스프라이트 기준)
  var exBase = Moghunter.theatrhythm_enemy_x + Moghunter.theatrhythm_button_layout_x - this._layout_buttons_data[0];
  var eyBase = Moghunter.theatrhythm_enemy_y + Moghunter.theatrhythm_button_layout_y - this._layout_buttons_data[1];
  this._sprite_enemy_buttons.setup_position(exBase, eyBase);
  this.createStrings();
};
...
Scene_Theatrhythm.prototype.update_commands = function () {
  // FNF 방향키만 사용: left=0, down=1, up=2, right=3
...
Scene_Theatrhythm.prototype.refresh_effect = function () {
  ...
  if (string_id != 0) {
    this.execute_hit_enemy(pos_id, string_id);
  } else {
    this.execute_missed();
  }
  $gameSystem._theatrhythm_key_data[0] = null;
...
Scene_Theatrhythm.prototype.execute_hit_enemy = function (pos_id, string_id) {
  ...
  if (this._result_data[0] < $gameSystem._theatrhythm_chain_data[0]) {
    this._result_data[0] = $gameSystem._theatrhythm_chain_data[0];
  }
  this.add_score();
...
Scene_Theatrhythm.prototype.execute_hit_actor = function (pos_id, string_id) {
  if (this._enemySprite._enemy._motion_action_data[0] != 0) {
    return;
  }
...
Scene_Theatrhythm.prototype.execute_damage = function (target) {
  ...
  if (target === 0) {
    ...
    if ($gameSystem._theatrhythm_key_data[1] == 2) {
      dmg *= Moghunter.theatrhythm_critical_bonus;
    }
    var realdmg = this.applyVariance(dmg, vdmg);
    this._enemy_hp[0] -= Math.floor(realdmg);
    if (this._enemy_hp[0] < 0) {
      this._enemy_hp[0] = 0;
    }
    if (this._enemy_hp[0] <= 0) {
      this._enemy_hp[0] = 0;
      this._enemySprite._enemy.performCollapse();
      this.battle_end(10);
    }
  } else {
...
Scene_Theatrhythm.prototype.add_score = function () {
  ...
  if ($gameSystem._theatrhythm_key_data[1] == 2) {
    value *= Moghunter.theatrhythm_critical_bonus;
  }
  this._score[1] += Math.floor(value);
  if ($gameSystem._theatrhythm_score_data[1] < this.score()) {
    $gameSystem._theatrhythm_score_data[1] = this.score();
  }
};
...
Scene_Theatrhythm.prototype.update_start_phase = function () {
  switch (this._phase[0]) {
    ...
    case 1:
      ...
      if (this._phase_sprite.scale.x > 1.0) {
    ...
    case 3:
      this._phase_sprite.opacity -= 3;
      this._phase_sprite.scale.x += 0.02;
      if (this._phase_sprite.opacity <= 0) {
        $gameSystem._theatrhythm_phase = 1;
        this._phase[0] = 4;
      }
      break;
  }
...
Scene_Theatrhythm.prototype.update_victory_phase = function () {
  switch (this._phase[1]) {
    ...
    case 6:
      ...
      if (this._phase_sprite.opacity <= 0) {
        this.fadeOutAll();
        $gameSystem._theatrhythm_phase = 12;
        this._phase[1] = 7;
        SceneManager.pop();
      }
      break;
...
Scene_Theatrhythm.prototype.gain_rewards = function () {
  if ($gameSystem._theatrhythm_switchID != 0) {
    $gameSwitches._data[$gameSystem._theatrhythm_switchID] = true;
  }
  if (!$gameVariables._data[$gameSystem._theatrhythm_variableID]) {
    $gameVariables._data[$gameSystem._theatrhythm_variableID] = 0;
  }
  if ($gameSystem._theatrhythm_variableID != 0) {
    $gameVariables._data[$gameSystem._theatrhythm_variableID] += this.set_total();
  }
...
Scene_Theatrhythm.prototype.update = function () {
  ...
  if (this._layout_buttons_data[0] === -1) {
    if (this._layout_buttons_img.isReady()) {
      this.getSpriteData();
    }
    return;
  }
  ...
  for (var i = 0; i < this._layout_buttons.length; i++) {
    ...
    if (this._strings[i].opacity > 0) {
      this._strings[i].scale.x += 0.05;
    }
    this._strings[i].scale.y = this._strings[i].scale.x;
  }
  if (!this._enemy_ref && this._enemySprite.bitmap.isReady()) {
    this.refresh_enemy_position();
  }
  if (this._meter_data[0] != -1) {
    this.update_time_meter();
  }
  if (this._hp_meter_data[0] != -1) {
    this.update_hp_meter();
  }
  if (this._point_data[0] != -1) {
    this.update_score();
  }
  if (this._enemyHpNumber_data[0] != -1) {
    this.update_enemyHpNumber();
  }
  if (this._actorHpNumber_data[0] != -1) {
    this.update_actorHpNumber();
  }
  if (this._chain_data[0] != -1) {
    this.update_chain();
  }
  if (this._damage_data[0] != -1) {
    this.update_damage();
  }
  if ($gameSystem._theatrhythm_phase === 1) {
  ...
  if ($gameSystem._theatrhythm_phase === 1) {
    this._fnfElapsed += 1000 / 60;
  }
  if ($gameSystem._theatrhythm_phase === 1 && this._sprite_enemy_buttons) {
    this._sprite_enemy_buttons.update();
  }
  if ($gameSystem._theatrhythm_miss_dmg) {
    this.execute_miss_damage();
  }
  if ($gameSystem._theatrhythmDebug[1]) {
    this.forceHit();
  }
  if (this._animeLag > 0) {
    this._animeLag -= 1;
...
//=============================================================================
function Spriteset_Theatrhythm() {
...
Sprite_TKeys.prototype.initialize = function (img, pos) {
  ...
  this._limit = [
    this._positions[0][0] + this._cw + this._cw / 4, // [0] OK 판정 좌끝
    this._positions[0][0] + this._cw * 3, // [1] OK 판정 우끝
    this._positions[0][0] + this._cw + this._cw, // [2] 크리티컬 좌끝
    this._positions[0][0] + this._cw * 2 + this._cw / 3, // [3] 크리티컬 우끝
    Graphics.boxWidth + this._cw, // [4] 스폰 위치(오른쪽)
  ];
  // FNF 채보 스폰용 변수
  this._fnfNextIdx = 0;
  // 노트가 스폰 위치→판정 중앙까지 이동하는 시간(ms) 계산
  var judgeCX = (this._limit[2] + this._limit[3]) / 2;
  var spawnX = -this._cw; // 왼쪽에서 스폰
  this._fnfTravelTime = ((judgeCX - spawnX) / Math.max(this._keysSpeed, 1)) * (1000 / 60);
  this.create_keys();
};
...
Sprite_TKeys.prototype.create_keys = function () {
  this._keys = [];
  this._keys_c = []; // 노트 방향 (판정 키)
  this._keys_f = []; // 히트됨
  this._keys_h = []; // 배틀러 인덱스
  this._keys_r = []; // 활성화 여부
  this._keys_s = []; // 슬로우패스(미스) 처리됨
  this._keys_speed = [];
  for (var i = 0; i < 16; i++) {
    this._keys_c[i] = 0;
...
Sprite_TKeys.prototype.keySpeed = function (i) {
  return this._keysSpeed + (this._keys_speed[i] || 0);
};

//==============================
// * KeysisValid — 방향키 입력 판정 (0=left,1=down,2=up,3=right)
//==============================
Sprite_TKeys.prototype.keyisValid = function (key) {
  ...
  // 판정선 범위에 맞는 노트 없음 → 미스
  if ($gameSystem._theatrhythm_key_data[0] == null) {
    $gameSystem._theatrhythm_key_data[0] = 0;
    $gameSystem._theatrhythm_key_data[1] = 0;
  }
};
...
Sprite_TKeys.prototype.execute_ok = function (i, key) {
  ...
  $gameSystem._theatrhythm_key_data[1] = this.checkKey(key, i, 1) ? 2 : 1;
  this._keys_f[i] = true;
};

//==============================
// * checkKey (0=OK범위, 1=크리티컬범위)
//==============================
Sprite_TKeys.prototype.checkKey = function (key, i, type) {
  if (!this._keys_r[i]) return false;
...
Sprite_TKeys.prototype.update = function () {
  ...
  if ($gameSystem._theatrhythm_fnfLoaded && $gameSystem._theatrhythm_phase === 1) {
    this.update_fnf_spawn();
  }
  for (var i = 0; i < this._keys.length; i++) {
    this.update_keys(i);
  }
  // 채보가 모두 끝났으면 자동 승리
  if ($gameSystem._theatrhythm_fnfLoaded && $gameSystem._theatrhythm_phase === 1) {
    this.check_chart_end();
  }
};

//==============================
// * Update FNF Spawn — 경과 시간에 맞춰 노트 스폰
//==============================
Sprite_TKeys.prototype.update_fnf_spawn = function () {
  var queue = $gameSystem._theatrhythm_fnfQueue;
  if (!queue || queue.length === 0) return;
  var elapsed = SceneManager._scene ? SceneManager._scene._fnfElapsed : 0;
  while (this._fnfNextIdx < queue.length) {
    var note = queue[this._fnfNextIdx];
    var spawnTime = note.time - this._fnfTravelTime;
    if (elapsed >= spawnTime) {
      this.spawn_fnf_note(note);
      this._fnfNextIdx++;
    } else {
      break;
    }
...
//==============================
// * Spawn FNF Note — 슬롯에 노트 배치
//==============================
Sprite_TKeys.prototype.spawn_fnf_note = function (note) {
  var slot = -1;
  for (var i = 0; i < this._keys.length; i++) {
    if (!this._keys_r[i]) {
      slot = i;
  ...
  if (slot < 0) return;
  var dir = note.direction; // 0=left,1=down,2=up,3=right
  var lane = dir % Math.max(this._batlers.length, 1);
  this._keys_c[slot] = dir;
  this._keys_h[slot] = lane;
  this._keys_f[slot] = false;
  this._keys_s[slot] = false;
  this._keys_r[slot] = true;
  var h = this._positions[lane] ? this._positions[lane][1] : this._positions[0][1];
  this._keys[slot].setFrame(this._cw * dir, 0, this._cw, this._ch);
  this._keys[slot].x = -this._cw; // 왼쪽에서 스폰
  this._keys[slot].y = h;
  this._keys[slot].scale.x = 1.0;
  this._keys[slot].scale.y = 1.0;
  this._keys[slot].opacity = 0;
};

//==============================
// * Check Chart End — 채보 완료 시 자동 승리
//==============================
Sprite_TKeys.prototype.check_chart_end = function () {
  var queue = $gameSystem._theatrhythm_fnfQueue;
  if (!queue || this._fnfNextIdx < queue.length) return;
  var elapsed = SceneManager._scene ? SceneManager._scene._fnfElapsed : 0;
  if (queue.length > 0 && elapsed < queue[queue.length - 1].time + 2000) return;
  for (var i = 0; i < this._keys.length; i++) {
    if (this._keys_r[i] && !this._keys_f[i] && this._keys[i].opacity > 0) return;
  }
  $gameSystem._theatrhythm_phase = 10;
  var sc = SceneManager._scene;
  if (sc && sc._battlers) {
    for (var j = 0; j < sc._battlers.length; j++) sc._battlers[j].performVictory();
  }
};
...
Sprite_TKeys.prototype.update_keys = function (i) {
  ...
  if (this.need_reset_key(i)) {
    this.reset_slot(i);
  }
};

//==============================
// * Update Position — 오른쪽→왼쪽 이동
//==============================
Sprite_TKeys.prototype.update_position = function (i) {
  this._keys[i].x += this.keySpeed(i); // 왼쪽→오른쪽
  if (this._keys[i].x < this._limit[0]) {
    // 아직 판정선 좌끝보다 왼쪽 → 서서히 나타남
    this._keys[i].opacity += 25;
    if (this._keys[i].opacity > 255) this._keys[i].opacity = 255;
  } else if (this._keys[i].x > this._limit[1]) {
    // 판정선 우끝을 지나침 → 미스
...
Sprite_TKeys.prototype.need_reset_key = function (i) {
  if (!this._keys_r[i]) return false;
  if (this._keys[i].x > this._limit[4]) return true; // 화면 오른쪽 끝 이탈
  if (this._keys_f[i] && this._keys[i].opacity <= 0) return true;
  if (!this._keys_f[i] && this._keys[i].opacity <= 0 && this._keys_s[i]) return true;
  return false;
};
...
// * Reset Slot
//==============================
Sprite_TKeys.prototype.reset_slot = function (i) {
  this._keys_r[i] = false;
  this._keys_f[i] = false;
  this._keys_s[i] = false;
  this._keys[i].x = -this._cw - 100;
...
Sprite_Actor.prototype.setActorHome = function (index) {
  if ($gameSystem._theatrhythm) {
    if (this._sprite_face) {
      this._sprite_face = null;
    }
    this.setHome(Moghunter.theatrhythm_actor_x, Moghunter.theatrhythm_actor_y + index * Moghunter.theatrhythm_actor_spc_y);
    return;
  }
...
Sprite_Actor.prototype.refreshMotion = function () {
  if ($gameSystem._theatrhythm) {
    this.refreshMotion_Theatrhythm();
    return;
  }
...
//=============================================================================
// ■■■ Sprite_TKeys_Enemy — 적 자동 노트 (180도 회전, 오른쪽→왼쪽) ■■■
//=============================================================================
function Sprite_TKeys_Enemy() {
  this.initialize.apply(this, arguments);
}
Sprite_TKeys_Enemy.prototype = Object.create(Sprite.prototype);
Sprite_TKeys_Enemy.prototype.constructor = Sprite_TKeys_Enemy;

//==============================
// * Initialize
//==============================
Sprite_TKeys_Enemy.prototype.initialize = function (img, pos) {
  Sprite.prototype.initialize.call(this);
  this._image = img;
  this._cw = this._image.width / 8;
  this._ch = this._image.height;
  ...
  this._posX = 0;
  this._posY = 0;
  // limit은 setup_position에서 설정
  this._limit = [0, 0, 0, 0, 0];
  this._keys = [];
  this._keys_c = [];
  this._keys_f = [];
  this._keys_r = [];
  ...
  for (var i = 0; i < 16; i++) {
    ...
    this._keys[i].anchor.x = 0.5;
    this._keys[i].anchor.y = 0.5;
    this._keys[i].rotation = Math.PI; // 180도 회전
    this._keys[i].opacity = 0;
    this.addChild(this._keys[i]);
  }
};
...
//==============================
// * Setup Position (getSpriteData 이후 호출)
// 적 노트는 오른쪽→왼쪽 이동이므로 판정선이 왼쪽에 있음
//==============================
Sprite_TKeys_Enemy.prototype.setup_position = function (px, py) {
  this._posX = px;
  this._posY = py;
  // 적 판정선: 적 위치 기준 (플레이어와 좌우 반전)
  // 노트가 오른쪽에서 왼쪽으로 오므로, 판정선은 px 근처
  this._limit = [
    px + this._cw + this._cw / 4, // [0] OK 판정 좌끝
    px + this._cw * 3, // [1] OK 판정 우끝
    px + this._cw + this._cw, // [2] 크리티컬 좌끝
    px + this._cw * 2 + this._cw / 3, // [3] 크리티컬 우끝
    Graphics.boxWidth + this._cw, // [4] 오른쪽 스폰 위치
  ];
  // 이동 시간: 오른쪽 끝→판정 중앙
  var judgeCX = (this._limit[2] + this._limit[3]) / 2;
  this._fnfTravelTime = ((this._limit[4] - judgeCX) / Math.max(this._keysSpeed, 1)) * (1000 / 60);
  // 대기 위치 초기화
  for (var i = 0; i < this._keys.length; i++) {
    this._keys[i].x = this._limit[4] + 100;
    this._keys[i].y = py;
  }
};
...
// * Update
//==============================
Sprite_TKeys_Enemy.prototype.update = function () {
  Sprite.prototype.update.call(this);
  if (!$gameSystem._theatrhythm_fnfLoaded) return;
  this.update_fnf_spawn();
  for (var i = 0; i < this._keys.length; i++) {
    this.update_key(i);
  }
...
// * Update FNF Spawn
//==============================
Sprite_TKeys_Enemy.prototype.update_fnf_spawn = function () {
  var queue = $gameSystem._theatrhythm_fnfEnemyQueue;
  if (!queue || queue.length === 0) return;
  var elapsed = SceneManager._scene ? SceneManager._scene._fnfElapsed : 0;
  while (this._fnfNextIdx < queue.length) {
    var note = queue[this._fnfNextIdx];
...
// * Spawn Note
//==============================
Sprite_TKeys_Enemy.prototype.spawn_note = function (note) {
  var slot = -1;
  for (var i = 0; i < this._keys.length; i++) {
    if (!this._keys_r[i]) {
      slot = i;
  ...
  if (slot < 0) return;
  var dir = note.direction % 4;
  this._keys_c[slot] = dir;
  this._keys_f[slot] = false;
  this._keys_s[slot] = false;
  this._keys_r[slot] = true;
  this._keys[slot].setFrame(this._cw * dir, 0, this._cw, this._ch);
  this._keys[slot].x = this._limit[4]; // 오른쪽에서 스폰
  this._keys[slot].y = this._posY;
  this._keys[slot].scale.x = 1.0;
  this._keys[slot].scale.y = 1.0;
  this._keys[slot].opacity = 0;
};
...
//==============================
// * Update Key — 자동으로 판정선 통과 (불이익 없음)
//==============================
Sprite_TKeys_Enemy.prototype.update_key = function (i) {
  if (!this._keys_r[i]) {
    this._keys[i].opacity = 0;
    return;
  }
  ...
  // 오른쪽→왼쪽 이동
  this._keys[i].x -= this._keysSpeed;
  if (this._keys[i].x > this._limit[1]) {
    this._keys[i].opacity += 25;
    if (this._keys[i].opacity > 255) this._keys[i].opacity = 255;
  } else if (this._keys[i].x < this._limit[0]) {
    // 판정선 통과 → 미스 (불이익 없음, 그냥 사라짐)
    this._keys[i].opacity -= 7;
    if (this._keys[i].opacity <= 0) {
      this.reset_slot(i);
    }
    ...
    this._keys[i].opacity = 255;
    if (!this._keys_f[i]) {
      this._keys_f[i] = true; // 자동으로 쳤다고 처리
    }
  }
  if (this._keys[i].x < -(this._cw * 2)) {
    this.reset_slot(i);
  }
};
...
// * Reset Slot
//==============================
Sprite_TKeys_Enemy.prototype.reset_slot = function (i) {
  this._keys_r[i] = false;
  this._keys_f[i] = false;
  this._keys[i].x = this._limit[4] + 100;
  this._keys[i].opacity = 0;
